...
import { createChart, CandlestickSeries, HistogramSeries, ColorType, CrosshairMode, LineStyle, type IChartApi, type ISeriesApi, type IPriceLine, type UTCTimestamp } from 'lightweight-charts'
...
const PALETTE = {
  dark: { bg: 'transparent', text: '#8b8f9c', grid: 'rgba(255,255,255,0.04)', border: 'rgba(255,255,255,0.08)' },
  light: { bg: 'transparent', text: '#6b6f7b', grid: 'rgba(15,16,20,0.05)', border: 'rgba(15,16,20,0.1)' },
}
const LONG = '#22c55e'
const SHORT = '#f43f5e'
...
export function TradingChart({ coin, szDecimals, positions, className }: { coin: string; szDecimals: number; positions: Position[]; className?: string }) {
  ...
  useEffect(() => {
    ...
    const c = createChart(el, {
      autoSize: true,
      layout: { background: { type: ColorType.Solid, color: 'transparent' }, textColor: PALETTE[theme].text, fontFamily: 'Inter, ui-sans-serif, system-ui', attributionLogo: false },
      grid: { vertLines: { color: PALETTE[theme].grid }, horzLines: { color: PALETTE[theme].grid } },
      crosshair: { mode: CrosshairMode.Normal },
      rightPriceScale: { borderColor: PALETTE[theme].border, scaleMargins: { top: 0.08, bottom: 0.22 } },
      timeScale: { borderColor: PALETTE[theme].border, timeVisible: true, secondsVisible: false, rightOffset: 4 },
  ...
  useEffect(() => {
    chart.current?.applyOptions({
      layout: { textColor: PALETTE[theme].text },
      grid: { vertLines: { color: PALETTE[theme].grid }, horzLines: { color: PALETTE[theme].grid } },
      rightPriceScale: { borderColor: PALETTE[theme].border },
      timeScale: { borderColor: PALETTE[theme].border },
    })
  }, [theme])
  ...
  useEffect(() => {
    ...
    fetchCandles(coin, interval, 400)
      .then((data) => {
        ...
        volume.current.setData(data.map((d) => ({ time: d.time as UTCTimestamp, value: d.volume, color: d.close >= d.open ? 'rgba(34,197,94,0.35)' : 'rgba(244,63,94,0.35)' })))
    ...
    const unsub = subscribeCandle(coin, interval, (c) => {
      ...
      volume.current.update({ time: c.time as UTCTimestamp, value: c.volume, color: c.close >= c.open ? 'rgba(34,197,94,0.35)' : 'rgba(244,63,94,0.35)' })
      last.current = c
    })
    return () => {
      alive = false
  ...
  return (
    <div className={cx('relative flex flex-col', className)}>
      ...
      <div ref={box} className="min-h-[280px] flex-1" />
      {loading && (
