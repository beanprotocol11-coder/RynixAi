...
/** Turns `ipfs://cid[/path]` into gateway URLs (first is preferred); passes http(s) URLs through. */
export function logoUrls(uri: string | null | undefined): string[] {
  if (!uri) return []
  const s = uri.trim()
  if (/^https?:\/\//i.test(s)) return [s]
  const m = /^ipfs:\/\/(?:ipfs\/)?(.+)$/i.exec(s)
  if (m) return IPFS_GATEWAYS.map((g) => `${g}/${m[1]}`)
  return []
}
...
/** Pons tokens store the creator's logo URI on-chain (`logo()`); used when GeckoTerminal has no image yet. */
export function readTokenLogo(token: Address): Promise<string | null> {
  const key = token.toLowerCase()
  let p = logoCache.get(key)
  if (!p) {
    p = publicClient
      .readContract({ address: token, abi: TOKEN_LOGO_ABI, functionName: 'logo' })
      .then((uri) => logoUrls(uri)[0] ?? null)
      .catch(() => null)
    logoCache.set(key, p)
  }
  return p
